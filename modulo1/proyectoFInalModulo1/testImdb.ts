...
let lolaDuenas: Professional = new Professional(
  "Lola Dueñas",
  49,
  65,
  165,
...
let imdb: Imdb = new Imdb([movie1, movie2, movie3, movie4, movie5, movie6]);
console.log('imdb');
imdb.peliculas.forEach(movie => movie.printMovie());

console.log('peliculasImportadas');
imdb.escribirEnFicheroJSON("imdbBBDD.json");

let peliculasImportadas: Imdb = new Imdb([]);
// console.log(peliculasImportadas.obtenerInstanciaIMDB('imdbBBDD.json'));

peliculasImportadas = peliculasImportadas.obtenerInstanciaIMDB("imdbBBDD.json");
if (peliculasImportadas != undefined) {
    peliculasImportadas.peliculas.forEach(pelicula => pelicula.printMovie());
} else {
console.log('peliculasImportadas es undefined');
