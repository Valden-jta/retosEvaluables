...
var fs = require("fs"); //para importar la funcionalidad writeFileSync Y readFileSync
...
var Imdb = /** @class */ (function () {
    ...
    Imdb.prototype.obtenerInstanciaIMDB = function (nombreFichero) {
        var data = JSON.parse(fs.readFileSync(nombreFichero, "utf8")); //  nos devuelve un objeto js, en este caso un objeto que contiene un array
        var peliculas = data.peliculas.map(function (pelicula) { return (0, class_transformer_1.plainToClass)(movie_1.Movie, pelicula); }); //  convertimos los datos del json en objetos tipo Movie, y lo incluimos en un array para generara un Movie[]
        var imdbInstance = new Imdb(peliculas); //  Ahora se puede usar ese array para instanciar un nuevo objeto Imdb
        return imdbInstance;
    };
    return Imdb;
}());
